...
//  MarvelsTests
//
//  Created by Savjot Singh on 02/02/22.
//
...
class CharacterModelTest: XCTestCase {
    ...
    {
        let expectation = XCTestExpectation.init(description: "Character API")
        httpManager.get(urlString: Constants.charactersURLString(), parameters: [Constants.kEmptyString:Constants.kEmptyString], completionBlock: {result in
            switch result {
            case .success( _):
